...
console.error("오류가 발생!! 메터js 로드 실패!!");
...
function addFruit(xPosition) {
    ...
    const fruitBody = createFruitBody(xPosition, PREVIEW_BALL_HEIGHT, currentDroppingFruitSizeIndex);
...
function handleFruitDrop(event) {
    ...
    addFruit(mousePosition.x);
}

function handleFruitPreviewMove(event) {
    // 게임 상태가 '준비'이고 미리보기 과일이 존재하면,
    if (currentGameState == GAME_STATE.READY || previewFruitBody) {
        // 미리보기 과일의 x좌표를 마우스의 x좌표로 업데이트하는 코드를 작성해봅시다!
        const mousePosition = event.mouse.position;
        Matter.Body.setPosition(previewFruitBody, { x: mousePosition.x, y: PREVIEW_BALL_HEIGHT });
    }
}
...
function calculateScore() {
    ...
    for (let i = 0; i < fruitsMergedCount.length; i++) {
        newScore += fruitsMergedCount[i] * FRUIT_DATA[i].scoreValue;
    }
...
function mergeFruits(fruit1, fruit2) {
    ...
    fruitsMergedCount[fruit1.sizeIndex] += 1;
    calculateScore();
...
function loseGame() {
    ...
    document.getElementById('game-retry-button').onclick = () => {
        location.reload();
    }
}
